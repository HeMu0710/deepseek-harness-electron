...
The host half scans enabled Loader entries for web `dsh.client` packages, resolves each `exports["./client"]`, hashes the built bundle into the boot graph, and exposes each absolute bundle path through `ctx.clientModules`. This inventory requires the Loader but not an HTTP server, so non-Web hosts can read `graph()` and resolve each row with `clientPath(id)`. While `ctx.webServer` is active, a child registration installs the Web carrier: it serves bundles and source maps under `/plugins` and injects the graph into the rendered index. Source launch maps host imports to TypeScript source but still consumes this built client export; missing files share one build instruction followed by a package/path list, while unrelated filesystem errors remain separate failures.
