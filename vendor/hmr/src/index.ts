...
class Hmr extends Service {
  ...
  constructor(ctx: Context, public config: Hmr.Config) {
    ...
    if (config.root.length > 0 && !this.internal) {
      throw new Error('Node internal module loader is required for module HMR')
    }
    this.baseDir = fileURLToPath(new URL(config.base || '.', ctx.baseUrl))
  }

  private requireInternal(): ModuleLoader {
  ...
  private async _resolve(specifier: string, parentURL: string, attrs: ImportAttributes): Promise<ResolveResult> {
    const internal = this.requireInternal()
  ...
  async* [Service.init]() {
    ...
    const internal = root.length === 0 ? undefined : this.requireInternal()
    ...
    const mainJob = internal?.loadCache.get(mainUrl)
    if (mainJob) {
  ...
  async getLinked(url: string) {
    const job = this.requireInternal().loadCache.get(url)
    if (!job) return []
