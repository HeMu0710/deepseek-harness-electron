...
> Division of labor: this document = the layering model + the channel-independent RPC protocol; the protocol's Web implementation combines HTTP uplink with the [WebSocket downlink carrier](2026-08-04-websocket-downlink-carrier.md), the [Electron desktop application](2026-08-14-electron-ipc-desktop-application.md) carries the same protocol through IPC, and the browser object layer is in the [web client architecture note](2026-07-19-gui-web-client-architecture.md).

## Problem

We need a UI integration layer. Beyond the existing ACP/stdio baseline, product clients include Web, Electron, and future surfaces. We call them Clients and want the following capabilities:
...
At the same time the physical channels differ per consumer (browser HTTP/WebSocket, in-process fetch/SSE, and Electron IPC Fetch/SSE), so we also need a channel-independent message model and a single contract source of truth — "adding a method" and "swapping a carrier" must not entangle each other, and every message on the wire must be type-validatable, observable, and reconcilable.
...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - `apps/electron` (`@deepseek-ai/dsh-desktop`) reuses the same Web shell and Client packages over a zero-port IPC Fetch carrier, with the Harness Host in an Electron utility process.
...
| Application | `@deepseek-ai/dsh` (apps/cli) + `dsh-web-frontend` (apps/web) + `@deepseek-ai/dsh-desktop` (apps/electron) | Profile dispatch, the Vite shell, and the Electron main/preload/utility-Host assembly | Each application owns its carrier and process lifecycle; workspace knowledge such as dist and packaged-resource location stays in the application |
...
1. **Pick a fetch impersonation**: browser same-origin HTTP / in-process `host.handler.fetch` injection / a transport-aspect subclass such as the Electron IPC implementation in the "Subclass table" below.
...
The three applications preserve the division: Web mounts its HTTP/WebSocket carrier and browser composition, Electron mounts a utility Host and zero-port IPC carrier, while `dsh --profile headless` mounts a direct core runner with zero Host, HTTP, or ports. ACP-class protocol bridges do not follow the client-carrier checklist: they expose core to the external ecosystem and mount directly via `ctx.plugin(entry-point plugin)` without fetch.
...
Two logical streams: the mux stream (`/api/events.mux`, all-session aggregate) and the host stream (`/api/events.host`, host-level events). The Web browser consumes one downlink WebSocket per stream, while the in-process and Electron IPC Fetch carriers retain SSE with the same event framing; see the [WebSocket downlink carrier](2026-08-04-websocket-downlink-carrier.md) and [Electron desktop application](2026-08-14-electron-ipc-desktop-application.md) for their physical boundaries. One example frame row:
...
| `DesktopApiClient` | dsh-client-connection | pull-based Fetch reconstruction over the narrow Electron preload bridge | swaps only `doFetch`; the base protocol and SSE decoder remain unchanged |
