...
The shipped bundles are `@deepseek-ai/dsh-base` (shared core rows), `@deepseek-ai/dsh-gui-app` (transport-independent GUI Host and Client roster), `@deepseek-ai/dsh-web-app` (Web carrier and runtime), `@deepseek-ai/dsh-electron-app` (desktop-native interaction with no Web rows), and `@deepseek-ai/dsh-headless` (a direct one-shot runner over base). Web composes `base + gui-app + web-app`, desktop composes `base + gui-app + electron-app`, and headless composes `base + headless`; the [Electron desktop decision](2026-08-14-electron-ipc-desktop-application.md) owns the desktop carrier and process lifecycle. Generic `dsh --profile <name>` hands its remaining arguments to that profile's command-line startup row: Web owns its flag family, while headless owns its task positional. Patch overlays use launcher-owned `--patch`. `dsh plugin --profile <name> <args...>` is a thin pnpm forwarder that initializes the profile and reconciles `dsh.profile.bundles` with installed bundle declarations; a package without a bundle declaration remains a plain dependency. [Headless as a direct core entry point](2026-08-09-headless-direct-core-entry-point.md) owns the headless composition contract.
