...
宿主侧会扫描已启用的 Loader 配置项以发现 web `dsh.client` 包，解析每个 `exports["./client"]`，把构建后的 bundle 哈希写入启动图，并通过 `ctx.clientModules` 公开每个 bundle 的绝对路径。该 inventory 依赖 Loader，但不依赖 HTTP 服务器，因此非 Web 宿主可以读取 `graph()`，并通过 `clientPath(id)` 解析每一行。`ctx.webServer` 活跃时，子级注册会安装 Web carrier：它通过 `/plugins` 提供 bundle 及其 sourcemap，并把该图注入渲染后的 index。源码启动会把宿主侧导入映射到 TypeScript 源码，但仍消费这一构建后的客户端导出；缺失文件共享一条构建说明，随后以包／路径列表列出各项，而无关的文件系统错误仍是独立故障。
