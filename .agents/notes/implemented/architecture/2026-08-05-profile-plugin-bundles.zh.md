...
随附的组合包是 `@deepseek-ai/dsh-base`（共享 core 配置项）、`@deepseek-ai/dsh-gui-app`（与传输无关的 GUI Host 与 Client roster）、`@deepseek-ai/dsh-web-app`（Web 载体与运行时）、`@deepseek-ai/dsh-electron-app`（不含 Web 配置项的桌面原生交互），以及 `@deepseek-ai/dsh-headless`（直接叠加在 base 上的一次性 runner）。Web 组合 `base + gui-app + web-app`，desktop 组合 `base + gui-app + electron-app`，headless 则组合 `base + headless`；[Electron 桌面决策](2026-08-14-electron-ipc-desktop-application.md)持有桌面载体和进程生命周期。通用的 `dsh --profile <name>` 把剩余参数交给该 profile 的命令行启动行：Web 持有自己的 flag，headless 则持有任务位置参数。patch overlay 使用启动器持有的 `--patch`。`dsh plugin --profile <name> <args...>` 是一层薄薄的 pnpm 转发器，负责初始化 profile，并依据已安装包的组合包声明调和 `dsh.profile.bundles`；没有组合包声明的包保持为普通依赖。[Headless 作为直接 core 入口](2026-08-09-headless-direct-core-entry-point.md)负责 headless 组合约定。
